...
| Fully autonomous factory self-construction claims | `partial` | Repository declares a staged maturity ladder and currently claims `provider-backed` readiness only; higher levels (`autonomous`) require additional objective evidence. | `docs/self-hosting-maturity-ladder.md`, `scripts/self-host-maturity.js`, `packages/cli/src/self-host-maturity.test.ts` |
| Unbounded unattended autonomous operation across external systems | `out-of-scope` | Current scope keeps autonomous operation gated behind objective higher-level evidence artifacts and does not claim unattended external-system autonomy. | `docs/self-hosting-maturity-ladder.md` (`AU-001`/`AU-002` gate requirements), `ROADMAP.md` |
