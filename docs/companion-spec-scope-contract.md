...
| Fully autonomous factory self-construction claims | `implemented` | Repository currently claims `full-autonomy` readiness only with objective FA-001 through FA-009 evidence and published readiness rollups. | `docs/self-hosting-maturity-ladder.md`, `scripts/self-host-maturity.js`, `packages/cli/src/self-host-maturity.test.ts`, `docs/metrics/reports/full-autonomy-readiness-latest.json` |
| Unbounded unattended autonomous operation across external systems | `out-of-scope` | Repository does not claim unattended external-system autonomy; full-autonomy readiness is declared with explicit out-of-scope boundaries for external-system autonomy. | `docs/roadmap/backlog-bk-018-full-autonomy-maturity-criteria.md`, `docs/self-hosting-maturity-ladder.md` (FA-* gates) |
